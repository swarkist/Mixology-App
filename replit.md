...
- **AI Endpoints Authentication Fix**: Resolved critical authentication bypass in AI endpoints by fixing middleware ordering (requireAuth before allowRoles) and adding missing routes (/api/openrouter, /api/youtube-transcript) with proper role-based access control.
- **Comprehensive Reviewer UI Restrictions**: Implemented consistent ReviewBanner and disabled save/delete functionality across all brand management workflows including BrandFromImageDialog, AddPreferredBrand, and EditPreferredBrand pages with proper styling (opacity-50, cursor-not-allowed).
